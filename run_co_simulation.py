...
def controller_function(power_set_point_hp: float, voltage: float, temperature: float) -> tuple[float, bool]:
    ...
    converge = voltage_min <= voltage <= voltage_max and temp_min <= temperature <= temp_max

    if converge:
        return power_set_point_hp, converge

    # Priority 1: Adjust power based on voltage limits
    if voltage < voltage_min:
        power_set_point_hp -= p_adjust_step_size_voltage
        print(f"Voltage {voltage} too low, decreasing power setpoint (consumption) to correct voltage.")
    elif voltage > voltage_max:
        power_set_point_hp += p_adjust_step_size_voltage
        print(f"Voltage {voltage} too high, increasing power setpoint (consumption) to correct voltage.")
    ...
    if voltage_min <= voltage <= voltage_max:
        if temperature > temp_max:
            ...
            print("Temperature is too high, reducing power setpoint of heatpump to cool down.")
        elif temperature < temp_min:
            power_set_point_hp += p_adjust_step_size_temp
            print("Temperature is too low, increasing power setpoint of heatpump to warm up.")
